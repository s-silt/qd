...
### 升级前必读：数据与老 HAR 模板兼容性

本 fork 相对官方版本的所有改动 **均为后向兼容**，从老版本升级 **不会破坏任何已有 HAR 签到模板**：
...
- `libs/fetcher.py` —— HTTP 请求重放引擎
- `libs/safe_eval.py` —— 变量沙箱
- `libs/cookie_utils.py` —— Cookie 会话
- `tpl` / `task` / `tasklog` 三张表的数据库 schema
- 模板里的 `har`、`env`、`variables` 字段格式
...
只是 WARNING，**不会阻止启动，老任务也会继续运行**。处理建议：

- **想保留所有老数据 → 别动 `AES_KEY`**：旧库里的 cookie / token / env 是用旧 key 加密的，换 key 后会**无法解密**。
- **想升级密钥** → 先用仓库根目录的 `backup.py` 把数据明文备份出来，再换 key 启动，最后重新导入。
- **`COOKIE_SECRET`** 可以随意更换，代价仅是所有用户被强制重新登录一次。
